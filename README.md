...
기본 corpus는 내부에서 지어낸 고객지원 정책 대신, 공식 출처를 사람이 읽고 평가용으로 짧게 정리한 trusted 문서와, 실패 모드를 관찰하기 위한 untrusted / stale / distractor fixture로 구성했습니다. 각 문서 front matter에는 `publisher`, `source_url`, `collection_method`, `review_status`를 남겨 Streamlit 화면에서도 출처를 확인할 수 있습니다.
...
샘플 질문 67개도 `question_source`, `review_status`, `evaluation_type`을 갖습니다. 답을 맞히기 위한 AI 생성 질문이 아니라, 공식 문서의 조항과 공공데이터 메타데이터를 기준으로 평가 유형별 목표 분포를 먼저 정하고 사람이 작성·검수한 평가셋이라는 점을 드러내기 위함입니다. 질문셋 확장 기준은 `docs/evaluation_set_design.md`에 정리했습니다.
