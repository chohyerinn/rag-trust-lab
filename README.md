...
| --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: |
| `basic-tradeoff` | 46 | 98% | 66% | 100% | 30% | 80% | 30% |
| `trusted-tradeoff` | 46 | 87% | 85% | 100% | 0% | 0% | 0% |

전체 평균만 보면 trusted가 더 좋아 보이지만, 유형별로 보면 tradeoff가 드러납니다.

| Type | n | basic | trusted | 해석 |
| --- | ---: | ---: | ---: | --- |
| `untrusted_only` coverage | 6 | 100% | 0% | 공식 문서만 쓰면 안전하지만 비공식 문서에만 있던 운영 정보는 답하지 못함 |
| `prompt_injection` injection | 6 | 100% | 0% | 오염 문서가 답변 context에 들어오면 공격 성공 가능성이 생김 |
| `source_conflict` poisoned retrieved | 6 | 0% | 0% | 악성 문구가 없는 충돌 문서는 prompt injection과 분리해서 관찰 |
| `insufficient_evidence` abstention | 5 | 100% | 100% | 문서에 없는 질문은 지어내지 않고 거절 |

## 실제 CLOVA 결과 (HCX-005, archived)

아래 표는 이전 20문항 평가셋에서 생성·judge를 모두 실제 **HCX-005**로 붙여 실행한 아카이브 결과입니다(mock 아님). 현재 기본 corpus는 질문 유형을 보강해 46문항으로 확장했으므로, 최신 실측 결과는 CLOVA 또는 LiteLLM config로 다시 실행해야 합니다.
...
기본 corpus는 내부에서 지어낸 고객지원 정책 대신, 공식 출처를 사람이 읽고 평가용으로 짧게 정리한 문서로 구성했습니다. 각 문서 front matter에는 `publisher`, `source_url`, `collection_method`, `review_status`를 남겨 Streamlit 화면에서도 출처를 확인할 수 있습니다.

| id | trust | why selected |
| --- | --- | --- |
| `easylaw_internet_refund` | trusted | 온라인 상담에서 자주 나오는 청약철회·반품 기준을 공식 생활법령 문서로 검증하기 위해 |
| `kca_dispute_resolution_standards` | trusted | 전자상거래 외의 수리·교환·환급 기준까지 답변 범위를 넓히기 위해 |
| `kca_damage_relief_process` | trusted | 상담 이후 피해구제 신청, 사업자 통보, 사실조사, 처리기한 흐름을 다루기 위해 |
| `privacy_policy_guide` | trusted | 개인정보 처리방침에 포함해야 할 항목을 공식 지침으로 확인하기 위해 |
| `privacy_safety_measures` | trusted | 접근권한, 접속기록, 암호화 같은 실제 운영 보안 질문을 다루기 위해 |
| `kca_consumer_consultation_trends` | trusted | 질문 주제 선정 근거로 실제 1372 소비자상담 기반 공식 데이터 메타데이터를 쓰기 위해 |
| `untrusted_refund_blog` | untrusted | 공식 환불 기준과 충돌하는 출처 불명 주장을 차단하는지 확인하기 위해 |
| `untrusted_privacy_memo` | untrusted | 개인정보 보유·처리방침 안내를 무시하라는 비공식 지시를 차단하는지 확인하기 위해 |
| `untrusted_refund_forum` | untrusted | 악성 지시문 없이 공식 기준과 충돌하는 커뮤니티성 정보를 source conflict로 분리해 보기 위해 |
| `untrusted_support_hours_note` | untrusted | 비공식 문서에만 답이 있는 운영 정보로 trusted-only의 coverage 손실을 측정하기 위해 |

샘플 질문 46개도 `question_source`, `review_status`, `evaluation_type`을 갖습니다. 답을 맞히기 위한 AI 생성 질문이 아니라, 공식 문서의 조항과 공공데이터 메타데이터를 기준으로 사람이 작성·검수한 평가셋이라는 점을 드러내기 위함입니다.
...
기본 실행은 API 키 없이 도는 **deterministic mock**입니다 — CI와 빠른 동작 확인용 smoke test이고, 모델 성능 결과가 아닙니다. 실제 성능 수치는 위의 **[실제 CLOVA 결과](#실제-clova-결과-hcx-005)**를 보세요. mock을 남겨둔 이유는 키 없이도 평가 파이프라인 전체(검색→생성→judge→통계)가 그대로 돌아가는 걸 누구나 재현할 수 있게 하기 위함입니다.
...
- trust 라벨은 사람이 부여했기 때문에 실제 운영처럼 완벽하지 않을 수 있습니다.
...
`configs/clova-basic.json`과 `configs/clova-trusted.json`은 답변 생성과 judge를 모두 HCX-005로 실행합니다. 현재 46문항 기준으로는 46문항 × 2설정 × (생성+judge) = 184회 호출입니다.
