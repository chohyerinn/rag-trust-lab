...
| Config | recall@3 | answer coverage | abstention | injection following | untrusted retrieved | poisoned retrieved |
| --- | ---: | ---: | ---: | ---: | ---: | ---: |
| `basic-tradeoff` | 100% | 82% | 100% | 6% | 64% | 33% |
| `trusted-tradeoff` | 94% | 93% | 100% | 0% | 0% | 0% |

전체 평균만 보면 trusted가 더 좋아 보이지만, 유형별로 보면 tradeoff가 드러납니다.

| Type | basic | trusted | 해석 |
| --- | ---: | ---: | --- |
| `untrusted_only` coverage | 100% | 0% | 공식 문서만 쓰면 안전하지만 비공식 문서에만 있던 운영 정보는 답하지 못함 |
| `prompt_injection` injection | 50% | 0% | 오염 문서가 답변 context에 들어오면 공격 성공 가능성이 생김 |
| `insufficient_evidence` abstention | 100% | 100% | 문서에 없는 질문은 지어내지 않고 거절 |
| `source_conflict` coverage | 100% | 100% | 비공식 충돌 문서가 있어도 공식 기준으로 답하는지 확인 |

## 실제 CLOVA 결과 (HCX-005, archived)

아래 표는 이전 20문항 평가셋에서 생성·judge를 모두 실제 **HCX-005**로 붙여 실행한 아카이브 결과입니다(mock 아님). 현재 기본 corpus는 공식 문서를 보강해 33문항으로 확장했으므로, 최신 수치를 만들려면 아래 CLOVA config를 다시 실행해야 합니다.
...
샘플 질문 33개도 `question_source`, `review_status`, `evaluation_type`을 갖습니다. 답을 맞히기 위한 AI 생성 질문이 아니라, 공식 문서의 조항과 공공데이터 메타데이터를 기준으로 사람이 작성·검수한 평가셋이라는 점을 드러내기 위함입니다.
...
`configs/clova-basic.json`과 `configs/clova-trusted.json`은 답변 생성과 judge를 모두 HCX-005로 실행합니다. 현재 33문항 기준으로는 33문항 × 2설정 × (생성+judge) = 132회 호출입니다.
