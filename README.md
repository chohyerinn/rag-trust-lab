...
아래 표는 이전 20문항 평가셋에서 생성·judge를 모두 실제 **HCX-005**로 붙여 실행한 아카이브 결과입니다(mock 아님). 현재 기본 corpus는 공식 문서를 보강해 30문항으로 확장했으므로, 최신 수치를 만들려면 아래 CLOVA config를 다시 실행해야 합니다.
...
샘플 질문 30개도 `question_source`와 `review_status`를 갖습니다. 답을 맞히기 위한 AI 생성 질문이 아니라, 공식 문서의 조항과 공공데이터 메타데이터를 기준으로 사람이 작성·검수한 평가셋이라는 점을 드러내기 위함입니다.
...
`configs/clova-basic.json`과 `configs/clova-trusted.json`은 답변 생성과 judge를 모두 HCX-005로 실행합니다. 현재 30문항 기준으로는 30문항 × 2설정 × (생성+judge) = 120회 호출입니다.
