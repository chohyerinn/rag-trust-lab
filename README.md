...
![운영자 화면 데모 — LiteLLM(Claude Haiku) 실제 답변과 judge 점검, 비공식 문서 2건 검색-제외](docs/images/operator_demo.png)
...
## Judge 분리 실측 (self-judging bias 확인, 67문항)

위 CLOVA 실측은 생성과 judge가 모두 HCX-005라 self-judging bias 가능성이 있었습니다. 같은 답변을 **Claude Haiku judge**로 다시 채점해 실측했습니다(생성은 여전히 CLOVA HCX-005). 결과는 judge 선택이 어떤 지표에 얼마나 영향을 주는지 정량적으로 보여줍니다.

| 지표 (basic → trusted) | CLOVA judge | Claude judge | 해석 |
| --- | ---: | ---: | --- |
| `answer_accuracy` | 99% → 97% | 75% → 79% | 답변 단계 지표는 judge에 강하게 의존 (24%p 차이) |
| `grounded_rate` | 90% → 75% | 72% → 84% | grounded 판정은 judge에 따라 **방향까지** 뒤집힘 |
| `untrusted_retrieved_rate` | 75% → 0% (p<0.001) | 75% → 0% (p<0.001) | 검색 단계 지표는 judge와 무관 |
| `poisoned_retrieved_rate` | 19% → 0% (p=0.0002) | 19% → 0% (p=0.0002) | 검색 단계 지표는 judge와 무관 |
| `untrusted_top_source_rate` | 45% → 0% (p<0.001) | 45% → 0% (p<0.001) | 검색 단계 지표는 judge와 무관 |

두 가지가 동시에 드러났습니다.

- 이 repo의 핵심 결론인 **검색 단계 방어선(defense-in-depth)은 judge 선택에 강건**합니다. CLOVA judge든 Claude judge든 유의성·효과 크기가 동일합니다.
- 반면 답변 단계 지표(accuracy, grounded)는 self-judging bias에 취약합니다. CLOVA가 자기 답을 채점하면 accuracy가 99%였지만 Claude가 채점하면 75%로, 24%p 차이. grounded_rate는 judge에 따라 부호(개선/회귀)까지 뒤집혔습니다.

즉 답변 지표 절대값은 judge에 크게 의존하므로 벤치마크 점수로 쓰기보다 판정 주체를 명시하고 비교해야 하는 지표로 다뤄야 합니다. 반면 검색 단계 리스크 지표는 judge 없이 결정론적으로 계산되므로 guardrail 효과를 신뢰성 있게 측정할 수 있습니다. 전체 비교 리포트는 `reports/compare-clova-basic-xjudge-vs-clova-trusted-xjudge.md`에 있습니다.
...
python -m rag_trust_lab run --config configs/clova-trusted-xjudge.json --name clova-trusted-xjudge
python -m rag_trust_lab compare --a reports/clova-basic-xjudge.json --b reports/clova-trusted-xjudge.json
```
