...
generator = "clova:HCX-005" if clova_key else "mock"
judge_mode = "clova:HCX-005" if clova_key else "heuristic"
...
with st.sidebar:
    ...
    st.caption(f"답변 생성: {_generator_label(generator)}")
    st.caption(f"답변 확인: {_judge_label(judge_mode)}")
    st.divider()
