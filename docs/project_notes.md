...
또 하나 어려웠던 점은 평가를 너무 크게 만들지 않는 것이었다. 처음에는 LangGraph, Qdrant, reranker, FastAPI까지 넣고 싶은 욕심이 있었지만, 그렇게 하면 완성도가 떨어질 것 같았다. 그래서 먼저 작은 문서셋, deterministic mock, Chroma/LiteLLM hook 정도로 범위를 줄였다.
...
### 2. 한국어 질문과 영어 문서가 섞이면 lexical 검색이 약했다

초기 샘플 문서는 영어 문장이 많았고 질문은 한국어였다. 그 결과 단순 lexical retriever가 관련 문서를 잘 못 찾는 경우가 있었다. 실제 RAG에서도 문서 언어와 질문 언어가 다르면 검색 품질이 흔들릴 수 있다는 걸 확인했다.

샘플 정책 문서에 한국어 문장을 함께 넣고, 이 문제를 README에서 “실제 모델 성능 주장이 아니라 평가 흐름을 보여주는 smoke test”라고 분명히 적었다.
...
trusted filtering을 켰을 때 accuracy가 오르거나 injection-following이 줄어드는 것처럼 보일 수 있지만, 문항 수가 20개라 우연일 수 있다. 그래서 `mini-agent-harness`에서 썼던 paired bootstrap CI와 McNemar test를 가져와서 유의성을 같이 보도록 했다.
...
- 실제 업무 문서처럼 더 긴 문서와 다양한 질문을 넣어보고 싶다.
