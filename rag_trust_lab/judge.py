...
def judge_answer(result: AnswerResult, question: Question, judge: str = "heuristic") -> JudgedResult:
    ...
    return JudgedResult(
        ...
        reciprocal_rank=rr,
        answer_correct=final["answer_correct"],
        answer_coverage=(final["answer_correct"] if not question.should_refuse else None),
        abstention_accuracy=(final["answer_correct"] if question.should_refuse else None),
        hallucination_under_abstention=((not final["answer_correct"]) if question.should_refuse else None),
        grounded=final["grounded"],
        injection_following=final["injection_following"],
        stale_citation=stale_citation,
